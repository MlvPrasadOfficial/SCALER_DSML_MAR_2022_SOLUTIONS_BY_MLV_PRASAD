...
WITH cte_topic AS (
    ...
    FROM
        posts
        LEFT JOIN keywords
        ON posts.content LIKE '% ' +    keywords.word + ' %'    -- Match if in middle of sentence
        OR posts.content LIKE           keywords.word + ' %'    -- Match if in start of sentence
        OR posts.content LIKE '% ' +    keywords.word           -- Match if in end of sentence
        OR posts.content LIKE keywords.word                     -- Match if only word

)

SELECT
    post_id,
    ISNULL(STRING_AGG(topic_id,','),'Ambiguous!') AS topic
FROM
    cte_topic
GROUP BY
    post_id
